...
### 自社の事業
SMS/AIBPOは障害福祉業界で以下の3事業 + 事業開発を行っている：
- **人材紹介**: 支援員・障害者雇用枠の両面
- **SaaS**: 福祉事業所向けソフトウェア
- **メディア**: 求職者・事業所マッチングメディア

### 4象限マトリクス（業界 × 提供価値）
競合を「業界が同じか × 提供価値が同じか」の2軸で4象限に分類する。
**重要: 各象限で知りたいことが全く異なる。**

```
              同じ提供価値              異なる提供価値
           ┌─────────────────┬─────────────────┐
 同じ業界  │ Q1: 直接競合(赤)     │ Q2: 市場探索(黄)     │
           │ OPSの深さを知る      │ 新市場を発見する      │
           ├─────────────────┼─────────────────┤
 異なる業界 │ Q3: OPS深化(青)     │ Q4: 技術キャッチ(紫)  │
           │ 隣接業界から学ぶ      │ AI/DXを追う          │
           └─────────────────┴─────────────────┘
```

### Q1: 直接競合 — 最も深い情報が必要
- 人材紹介/SaaS/メディアのサブカテゴリ別に分類
- **知りたいこと**: 売上・顧客数・利益率・人員体制・広告の出し方・営業手法・サポート体制・お客さんの声
- **将来的には**: ビジネスモデル図（BPMN風でステークホルダー×サービスのやり取りが見える形）
- **問い**: 自社と同じ市場で誰がどれだけシェアを持っているか？差別化の余地は？

### Q2: 市場探索 — ビジネスモデル発見が目的
- 事業所向け/企業向け/独自路線のサブカテゴリ
- **知りたいこと**: 誰の・何のニーズに刺しているか / 市場規模（TAM） / ビジネスモデル
- **問い**: SMS/AIBPOが参入する余地はあるか？自社アセットを活かせるか？

### Q3: OPS深化 — 隣接業界からの学び
- 介護/医療/SaaS/HR・メディアの業界別に分類
- **知りたいこと**: 組織運営・OPSの作り方 / 事業変遷 / 市場規模と勢力図
- **問い**: 隣接業界で成功しているOPSモデルは障害福祉に転用できるか？

### Q4: 技術キャッチアップ — 技術動向の把握
- AI/BPO、営業AI、EdTech等
- **知りたいこと**: 採用技術 / 事業化手法 / 自社への適用可能性
- **問い**: AIBPOが到来する中で、何の技術を最優先でキャッチアップすべきか？
...
### 情報フローの原則
新入社員が「この事業って何？」→「どうやって運営するの？」→「儲かるの？」の順で理解できるフロー。
情報量は多くてよいが、**流れ（ストーリー）が大事**。

### 4タブ構成と各タブの役割
```
📊 市場系（青）→ 📜 沿革系（黄）→ 💰 経営系（緑）→ 🔧 業務プロセス理解（紫）
```

1. **市場系**: 「この事業の市場はどれくらい？」— KPI/参入法人/規模/推移/地域分布
2. **沿革系**: 「制度はどう変わってきた？」— 報酬改定の歴史と影響
3. **経営系**: 「儲かるの？どう始めるの？」
   - **PL + 月次PL が最上部**（★一番大切）— 収支構造と月次の金の流れ
   - **事業ライフサイクル**（開業フロー埋込済み）— 開業から成長までの全体像
   - **報酬単位 + 加算一覧**（処遇改善フロー統合済み）— 制度の仕組みと加算取得
4. **業務プロセス理解**: 「現場はどうなってるの？」
   - **登場人物が最初**（★まず「誰がいるの？」を理解）
   - **一日の流れ + 会話シーン統合**（具体的な時間割と現場の声が一体化）
   - **業務プロセスマップ** — フェーズ別タスク俯瞰
   - **利用者フロー** — 利用者視点での日次/ライフサイクル
   - **ステークホルダーマップ** — 関係者間のネットワーク

### 全19サービス共通コンポーネント
- `FacilityDetailPage.tsx` — タブUI + セクション描画の共通レイアウト
- `FACILITY_SECTION_GROUPS` (constants.ts) — タブ/セクション構成の定義
- 各セクションはデータ有無で自動非表示（`OPTIONAL_SECTION_CHECKS`）
- データは `data/facility-analysis/*.json`（Python生成）
...
- `components/company/ChaosMap.tsx` — 2×2マトリクスレイアウト
  - Y軸: 同じ業界/異なる業界、X軸: 同じ提供価値/異なる提供価値
  - 各象限カードクリック → `/company/quadrant/{slug}` 遷移
  - 未分類企業（62社）は下部に折りたたみ表示
  - KPIカード4枚（総企業数/4象限/分類済み/高脅威企業）
- `components/company/QuadrantDetailPage.tsx` — 象限別企業カード一覧
- `app/company/quadrant/[slug]/page.tsx` — generateStaticParams で4ページSSG
- `lib/types.ts` — `Quadrant = "Q1"|"Q2"|"Q3"|"Q4"`, `Company.quadrant?` 追加
- `lib/constants.ts` — `QUADRANT_CONFIG`, `QUADRANT_SLUG_MAP`, NAV_ITEMS順序変更
- `lib/data.ts` — `getCompaniesByQuadrant()` 追加
- 20社を仮分類: Q1(8), Q2(4), Q3(5), Q4(3)

### 4象限の定義と戦略的意図（★重要 — ページ設計の根幹）

| 象限 | 業界 | 提供価値 | ラベル | 色 | 目的 |
|------|------|---------|--------|-----|------|
| Q1 | 同じ | 同じ | 直接競合 | #EF4444 (赤) | OPS・売上・人員体制まで深く知る |
| Q2 | 同じ | 違う | 市場探索 | #F59E0B (琥珀) | 事業開発として新市場を発見する |
| Q3 | 違う | 同じ | OPS深化 | #3B82F6 (青) | 隣接業界の運営ノウハウを学ぶ |
| Q4 | 違う | 違う | 技術キャッチアップ | #8B5CF6 (紫) | AI/DXの最新をキャッチアップ |

### ★ 各象限ページで見たい情報（段階的に実装）

**共通**: 象限の定義カード（なぜこの象限を監視するか）+ 企業ランキング表 + 企業カード一覧

**Q1: 直接競合** — 最も深い情報が必要
- ビジネスモデル図（BPMN風: ステークホルダー×サービスのやり取りが視覚的に見える形）
- 具体OPS: 売上/顧客数/利益率/人員体制/広告/営業方法/サポート体制/お客さんの声
- サービス別ランキング表（人材紹介/SaaS/メディアごとに誰が何位か）
- 経営戦略サマリー

**Q2: 市場探索** — ビジネスモデル発見が目的
- 誰の/何のニーズに刺しているか
- 市場規模（TAM/SAM/SOM）
- ビジネスモデル概要（課金モデル/顧客セグメント/チャネル）
- 新規参入可能性の評価

**Q3: OPS深化** — 隣接業界からの学び
- 業界別勢力図（介護/保育/医療/SaaS横断の市場ポジション）
- 組織運営/OPSの作り方
- 事業立ち上げ→現在の土俵を築くまでの変遷
- 市場規模と業界構造
...
## ★ Phase 23 完了（2026-02-23）: 事業所分析ページ4タブUI化

### 概要
事業所分析の各サービス詳細ページ（全19ページ）を4タブUIに分割。
17セクションの超長尺ページ（推定6000-9000px）を4タブに分離し、スクロール量を1/4に削減。
...
- `components/facility/FacilityDetailPage.tsx` — タブUI化（`useState`でタブ切替、`"use client"` 追加）

### 設計ポイント
- **タブUI**: `useState`でactiveTab管理、条件レンダリングでタブ内容切替
- タブバー: sticky + backdrop-blur、アイコン + ラベル + セクション数バッジ + カラーインジケーター
- SectionNavはタブ内ローカルナビとして動作（2セクション以上のタブのみ表示）
- モバイル対応: sm未満では短縮ラベル（市場/沿革/経営/業務）表示
